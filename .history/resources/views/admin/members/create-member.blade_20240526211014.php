...
@section('custom-scripts')
    ...
    $('#addFileInput').click(function () {
        // Code to add a new file input dynamically...

        // Add change event listener to all dynamically added file inputs
        $('.file-input').on('change', function (event) {
            $(this).closest('.col-md-6').find('.preview').attr('src', URL.createObjectURL(event.target.files[0]))
        });
    });
});


        // Function to dynamically add more file upload fields
        $(document).ready(function () {
            $('#addFileInput').click(function () {
                ...
                $('.file-input').last().on('change', function (event) {
                    $(this).closest('.col-md-6').find('.preview').attr('src', URL.createObjectURL(event.target.files[0]))
                });
            });
        });
